...
class CreateRutePerjalanan implements ShouldQueue
{
    ...
    {
        $s = explode(":", $this->request['date_start']);
        //tipe transportasi
        $transportation_type = $this->request['type_transportation'];
        $this->setTransportationType($transportation_type);

        //jumlah hari dari rentang tanggal
        $tanggal_awal = strtotime($s[0]);
        $tanggal_akhir = strtotime($this->request['date_end']);
        ...
        //set tanggal akhir
        $this->setTanggalAkhir($this->request['date_end']);
        ...
        $this->setNameRouteTravel($this->request['name_route_travel']);

        //set range date
        $Date = $this->getDatesFromRange($s[0], $this->request['date_end']);
        $this->setRangeDate($Date);

        //set hours in every destination
        $this->setHoursInEveryDestination($this->request['hours']);
        ...
        //set gambar
        $this->setGambar($this->request['gambar']);

        $this->setUserId($this->request['user_id']);

        //save to table plan
        $plan = new Plan();
        $plan->user_id = $this->request['user_id'];
        $plan->name_route_travel = $this->request['name_route_travel'];
        $plan->count_destination = $this->destination_length;
        $plan->date = $this->tanggal_awal . " - ". $this->tanggal_akhir;
        $plan->type_transportation = $this->request['type_transportation'];
        ...
        //untuk testing
        $category = array();
        for($i=0 ; $i< count($this->request['category_wisata']); $i++){
            array_push($category, $this->request['category_wisata'][$i]);
        }
