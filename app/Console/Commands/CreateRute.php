...
class CreateRute extends Command
{
    protected $signature = 'rute:create';

    protected $description = 'make rute perjalanan per 5 minute';
    ...
    public function handle()
    {
        ...
        foreach($data as $d){
            if($d->status == 0){
                ...
                //set tanggal akhir
                $this->setTanggalAkhir($d->date_end);
                ...
                $this->setNameRouteTravel($d->name_route_travel);

                //set range date
                $Date = $this->getDatesFromRange($s[0], $d->date_end);
                $this->setRangeDate($Date);

                //set hours in every destination
                $this->setHoursInEveryDestination($d->hours);
                ...
                //set gambar
                $this->setGambar($d->gambar);

                $this->setUserId($d->user_id);


                //untuk testing
                $category = array();
                for($i=0 ; $i< count(json_decode($d->category_wisata)); $i++){
                    array_push($category, json_decode($d->category_wisata[$i]));
                }
